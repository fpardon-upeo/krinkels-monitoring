...
private class GanttRoundPinActionTest {
    private static Map<String, Object> setupTestData() {
        ...
        Contact testContact = FieldServiceTestData.createTestContact(
                'Test',
                true
        );
...
#### Signature
```apex
private static void testSuccessfulRescheduling()
```
...
### `testMultipleAppointmentsWithSharedResources()`

`ISTEST`

#### Signature
```apex
private static void testMultipleAppointmentsWithSharedResources()
```
...
### `testDifferentRecurrencePatterns()`
...
#### Signature
```apex
private static void testDifferentRecurrencePatterns()
```
